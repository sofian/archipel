...
/// Audio pin outputs.
#define AUDIO_1_OUTPUT 11
#define AUDIO_2_OUTPUT 12

/// Open analog pin for random init.
#define RANDOM_AIN         A0
...
#define SILENT_AUTO_PLAY_DELAY_MIN   60000UL
#define SILENT_AUTO_PLAY_DELAY_MAX   90000UL
#define SILENT_AUTO_PLAY_DELAY_NOISE 10000UL

/* FOR DEBUGGING
#define SILENT_AUTO_PLAY_DELAY_MIN   5000UL
#define SILENT_AUTO_PLAY_DELAY_MAX   6000UL
#define SILENT_AUTO_PLAY_DELAY_NOISE 1000UL
*/
...
/// Sleep control.
#define SLEEP_POWER_DOWN_TIME    50
#define SLEEP_LISTEN_N_RETRIES  500
...
/// Millisecond counter.
volatile unsigned long __controlCounter = 0;
...
// Variables for silent-mode auto-play.
unsigned long silentAutoPlayMillis;
unsigned long silentAutoPlayDelay;
...
//  SERIAL_IR.begin(SERIAL_IR_BAUDRATE);
  ...
  silentAutoPlayMillis = 0;
  silentAutoPlayDelay = 0;
...
//const unsigned long SLEEP_CONTROL_COUNTER_INC = (unsigned long)(SLEEP_POWER_DOWN_TIME / 1000.0f * CONTROL_RATE);
...
//      Q_ARRAY_COPY(messageDna.code, (uint8_t*)&hash, uint8_t, info.byteSize());
  ...
  //    dna.copyFrom(messageDna);
      ...
      // Start trigger timer.
      triggerDelay = timerDelay(GENE_TRIGGER_DELAY, TRIGGER_DELAY_MIN, TRIGGER_DELAY_MAX, TRIGGER_DELAY_NOISE);
      triggerTimer.start();

      // Restart auto-play timer.
      autoPlayDelay = timerDelay(GENE_AUTO_PLAY_DELAY, AUTO_PLAY_DELAY_MIN, AUTO_PLAY_DELAY_MAX, AUTO_PLAY_DELAY_NOISE);
      autoPlayTimer.start();
...
/*  else if (digitalRead(IR_IN) == LOW) // message incoming: just wake up
  {
    autoPlay();
  }
  */
...
void loop() {
  // Check if we need to be in silent / sleeping mode.
  if (silentModeTimer.currentTime() >= SILENT_MODE_DELAY)
  {
    // Interrupt Mozzi and stop audio upon entering sleep mode.
    if (!hasPaused) {
      // Pause.
      pauseMozzi();
      digitalWrite(AUDIO_1_OUTPUT, LOW);
      digitalWrite(AUDIO_2_OUTPUT, LOW);
      hasPaused = true;
      
      // Restart auto-play timer.
      silentAutoPlayDelay = timerDelay(GENE_AUTO_PLAY_DELAY, SILENT_AUTO_PLAY_DELAY_MIN, SILENT_AUTO_PLAY_DELAY_MAX, SILENT_AUTO_PLAY_DELAY_NOISE);
      silentAutoPlayMillis = 0;
    }
...
//    digitalWrite(INDICATOR_LED_OUT, LOW);
    ...
    silentAutoPlayMillis += SLEEP_POWER_DOWN_TIME;
    
    // Check for incoming message.
    // NOTE: I cannot use timers here cause they won't work right after powering down.
    bool triggerPlay = (silentAutoPlayMillis >= silentAutoPlayDelay);
    if (!triggerPlay) 
    {
    ...
    // Play has triggered: restart audio system and play tune.
    if (triggerPlay) {
      unPauseMozzi(); 
      hasPaused = false;
      silentModeTimer.start();

      // TODO: pas besoin de ces deux lignes a mon avis (c'est redondant)
      silentAutoPlayDelay = timerDelay(GENE_AUTO_PLAY_DELAY, SILENT_AUTO_PLAY_DELAY_MIN, SILENT_AUTO_PLAY_DELAY_MAX, SILENT_AUTO_PLAY_DELAY_NOISE);
      silentAutoPlayMillis = 0;
